...
def fetch_library_hashes(src: FetchUrl) -> Dict[str, FetchUrl]:
    ...
    def library_src(library: Dict[str, Any]):
        ...
        return FetchUrl(
            name=os.path.basename(artifact["url"]),
            url=artifact["url"],
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=THREADS) as p:
            for game_version, version, src, library_srcs in p.map(
                ...
                for name, src in library_srcs.items():
                    if name not in library_versions:
                        library_versions[name] = src
...
if __name__ == "__main__":
    ...
    with (
        ...
        open(li, "r") as library_locks,
    ):
        loader_versions = {} if llo.stat().st_size == 0 else json.load(loader_locks)
        game_versions = {} if glo.stat().st_size == 0 else json.load(game_locks)
        library_versions = {} if li.stat().st_size == 0 else json.load(library_locks)
